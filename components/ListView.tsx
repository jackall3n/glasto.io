...
export default function ListView({ performances, users }) {
  const days = useMemo(() => {
    const ids = users.map(({ choices }) => choices).flat(Infinity) as string[];
    const uniq = uniqBy(ids);
    const mapped = uniq.map(id => performances.find((p) => p.id === id)).filter(Boolean);

    const ordered = orderBy(mapped, 'start');
